...
library(ggplot2)
library(dplyr)
mydata = read.table("Data/Bioinformatics Data.txt", sep="\t", header=TRUE)
mydata <- mydata %>% filter(Row == "A" & Compound %in% c("Mock", "Piplartine"))
ggplot(data = mydata, mapping = aes(x = Timepoint, y = Nuclei_IntNucGFP_Mean_Mean_per_Well, by = Compound, color = Compound)) + geom_point()
```
